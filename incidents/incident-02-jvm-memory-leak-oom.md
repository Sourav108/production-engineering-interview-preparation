# Incident Scenario: JVM Native DirectBuffer Leak and Linux OOM Kill

**Category**: Memory Leak / OOM | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (memory leak / oom).
...
2. *Hypothesis B (Memory Leak / OOM)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in memory leak / oom crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
