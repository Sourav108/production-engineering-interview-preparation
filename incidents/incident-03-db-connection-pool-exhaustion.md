# Incident Scenario: HikariCP Database Connection Pool Exhaustion

**Category**: DB Pool Exhaustion | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (db pool exhaustion).
...
2. *Hypothesis B (DB Pool Exhaustion)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in db pool exhaustion crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
