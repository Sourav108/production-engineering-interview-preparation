# Incident Scenario: Tomcat Thread Pool Starvation on Slow Third-Party API

**Category**: Thread Starvation | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (thread starvation).
...
2. *Hypothesis B (Thread Starvation)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in thread starvation crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
