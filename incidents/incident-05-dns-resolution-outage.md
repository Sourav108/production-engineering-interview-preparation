# Incident Scenario: CoreDNS Pod CPU Throttling and Resolution Timeouts

**Category**: DNS Outage | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (dns outage).
...
2. *Hypothesis B (DNS Outage)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in dns outage crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
