# Incident Scenario: Edge Ingress TLS Certificate Expiration

**Category**: TLS Failure | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (tls failure).
...
2. *Hypothesis B (TLS Failure)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in tls failure crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
