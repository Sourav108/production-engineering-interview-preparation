# Incident Scenario: Reverse Proxy 504 Gateway Timeout Cascade

**Category**: LB / Proxy Failure | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (lb / proxy failure).
...
2. *Hypothesis B (LB / Proxy Failure)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in lb / proxy failure crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
