# Incident Scenario: Read Replica Replication Lag Causing Balance Inconsistencies

**Category**: Replication Lag | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (replication lag).
...
2. *Hypothesis B (Replication Lag)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in replication lag crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
