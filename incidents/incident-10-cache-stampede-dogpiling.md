# Incident Scenario: Redis Cache Stampede on Global Homepage Banner Key

**Category**: Cache Outage | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (cache outage).
...
2. *Hypothesis B (Cache Outage)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in cache outage crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
