# Incident Scenario: Kafka Consumer Group Rebalance Loop and Queue Backlog

**Category**: Queue Backlog | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (queue backlog).
...
2. *Hypothesis B (Queue Backlog)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in queue backlog crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
