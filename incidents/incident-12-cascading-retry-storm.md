# Incident Scenario: Client-Side Unbounded Retries Inducing Retry Storm

**Category**: Retry Storm | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (retry storm).
...
2. *Hypothesis B (Retry Storm)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in retry storm crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
