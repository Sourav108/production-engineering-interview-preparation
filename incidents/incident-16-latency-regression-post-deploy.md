# Incident Scenario: High p99 Latency Regression after ORM N+1 Query Release

**Category**: Latency Regression | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (latency regression).
...
2. *Hypothesis B (Latency Regression)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in latency regression crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
