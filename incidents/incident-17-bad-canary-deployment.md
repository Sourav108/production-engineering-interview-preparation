# Incident Scenario: Canary Release NullPointerException on iOS User Segment

**Category**: Bad Deployment | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (bad deployment).
...
2. *Hypothesis B (Bad Deployment)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in bad deployment crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
