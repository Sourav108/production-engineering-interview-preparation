# Incident Scenario: Environment Variable Typo Causing Pod CrashLoopBackOff

**Category**: Config Regression | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (config regression).
...
2. *Hypothesis B (Config Regression)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in config regression crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
