# Incident Scenario: File Descriptor Exhaustion from Unclosed HTTP Clients

**Category**: FD Exhaustion | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (fd exhaustion).
...
2. *Hypothesis B (FD Exhaustion)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in fd exhaustion crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
