# Incident Scenario: AWS Availability Zone Power Outage and Evacuation

**Category**: Partial Region Outage | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (partial region outage).
...
2. *Hypothesis B (Partial Region Outage)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in partial region outage crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
