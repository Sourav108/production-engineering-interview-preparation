# Incident Scenario: High Cardinality Metric Explosion Crashing Prometheus TSDB

**Category**: Monitoring Outage | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (monitoring outage).
...
2. *Hypothesis B (Monitoring Outage)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in monitoring outage crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
