# Incident Scenario: Floating Point Rounding Error in Ledger Balance Computation

**Category**: Silent Corruption | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (silent corruption).
...
2. *Hypothesis B (Silent Corruption)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in silent corruption crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
