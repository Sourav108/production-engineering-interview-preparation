# Incident Scenario: Payment Webhook Duplicate Delivery Triggering Double Charge

**Category**: Duplicate Requests | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (duplicate requests).
...
2. *Hypothesis B (Duplicate Requests)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in duplicate requests crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
