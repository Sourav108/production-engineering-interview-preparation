# Incident Scenario: Cold Cache Pod Restart Triggering Thundering Herd on DB

**Category**: Cascading Failure | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (cascading failure).
...
2. *Hypothesis B (Cascading Failure)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in cascading failure crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
