# Incident Scenario: Kubelet Pressure Eviction Causing Node NotReady Cascades

**Category**: Node Failure | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (node failure).
...
2. *Hypothesis B (Node Failure)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in node failure crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
