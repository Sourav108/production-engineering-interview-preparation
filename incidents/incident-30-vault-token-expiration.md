# Incident Scenario: HashiCorp Vault ServiceAccount Token Expiry Causing 401s

**Category**: Auth Secret Expiry | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (auth secret expiry).
...
2. *Hypothesis B (Auth Secret Expiry)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in auth secret expiry crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
