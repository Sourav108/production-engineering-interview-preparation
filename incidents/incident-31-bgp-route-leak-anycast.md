# Incident Scenario: BGP Route Leak Causing Global Edge CDN Traffic Drops

**Category**: Network Routing | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (network routing).
...
2. *Hypothesis B (Network Routing)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in network routing crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
