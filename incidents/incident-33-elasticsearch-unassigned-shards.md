# Incident Scenario: Elasticsearch Cluster Red Status from Out-of-Disk Nodes

**Category**: Search Cluster Red | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (search cluster red).
...
2. *Hypothesis B (Search Cluster Red)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in search cluster red crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
