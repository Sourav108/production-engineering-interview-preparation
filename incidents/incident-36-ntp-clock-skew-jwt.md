# Incident Scenario: NTP Clock Drift on Auth Nodes Rejecting Valid JWT Tokens

**Category**: Clock Skew | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (clock skew).
...
2. *Hypothesis B (Clock Skew)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in clock skew crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
