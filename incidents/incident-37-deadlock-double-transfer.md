# Incident Scenario: Concurrent Bidirectional Money Transfer Database Deadlock

**Category**: DB Deadlock | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (db deadlock).
...
2. *Hypothesis B (DB Deadlock)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in db deadlock crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
