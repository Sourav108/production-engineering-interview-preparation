# Incident Scenario: Autovacuum Worker Blocked by Long-Running Reporting Query

**Category**: PostgreSQL Bloat | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (postgresql bloat).
...
2. *Hypothesis B (PostgreSQL Bloat)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in postgresql bloat crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
