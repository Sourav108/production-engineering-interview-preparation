# Incident Scenario: Ingress NGINX Worker File Descriptor Exhaustion on Config Reload

**Category**: Ingress Exhaustion | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (ingress exhaustion).
...
2. *Hypothesis B (Ingress Exhaustion)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in ingress exhaustion crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
