# Incident Scenario: CoreDNS Overloaded by High-Frequency Negative Cache Queries

**Category**: DNS Amplification | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (dns amplification).
...
2. *Hypothesis B (DNS Amplification)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in dns amplification crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
