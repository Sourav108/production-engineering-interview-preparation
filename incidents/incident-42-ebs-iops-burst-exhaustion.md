# Incident Scenario: AWS EBS gp3 Volume IOPS Burst Bucket Exhaustion

**Category**: Storage IOPS Burst | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (storage iops burst).
...
2. *Hypothesis B (Storage IOPS Burst)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in storage iops burst crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
