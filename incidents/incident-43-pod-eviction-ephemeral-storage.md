# Incident Scenario: Container Ephemeral Storage Exhaustion from Verbose Local Logs

**Category**: Pod Eviction | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (pod eviction).
...
2. *Hypothesis B (Pod Eviction)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in pod eviction crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
