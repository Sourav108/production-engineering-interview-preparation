# Incident Scenario: Misconfigured Service Mesh Routing Doubling Cross-AZ Latency

**Category**: Cross-AZ Latency | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (cross-az latency).
...
2. *Hypothesis B (Cross-AZ Latency)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in cross-az latency crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
