# Incident Scenario: Istio Envoy Sidecar Out-of-Memory Crash on High Connection Concurrency

**Category**: Service Mesh OOM | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (service mesh oom).
...
2. *Hypothesis B (Service Mesh OOM)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in service mesh oom crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
