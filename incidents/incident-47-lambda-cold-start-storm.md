# Incident Scenario: Serverless Cold Start Concurrency Spike Saturation

**Category**: Serverless Spikes | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (serverless spikes).
...
2. *Hypothesis B (Serverless Spikes)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in serverless spikes crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
