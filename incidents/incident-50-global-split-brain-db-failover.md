# Incident Scenario: Automated Failover Split-Brain in Asynchronous PostgreSQL Cluster

**Category**: Global Split-Brain | **Severity**: Sev-1 | **Target Level**: Staff / Senior Production Engineer
...
- **14:14**: Diagnostic investigation isolated root cause (global split-brain).
...
2. *Hypothesis B (Global Split-Brain)*: Tested via telemetry queries $	o$ **CONFIRMED**.
...
## 10. Root Cause
Resource contention in global split-brain crossed non-linear saturation thresholds under peak concurrency, queueing in-flight worker threads.
